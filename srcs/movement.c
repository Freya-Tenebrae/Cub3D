...
/*   Updated: 2022/09/09 12:50:43 by cmaginot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move(t_maps **maps)
{
	if ((*maps)->key_pressed[KEY_W] == 1)
		move_on_map(maps, 'w', KEY_MOVE);
	if ((*maps)->key_pressed[KEY_A] == 1)
		move_on_map(maps, 'a', KEY_MOVE);
	if ((*maps)->key_pressed[KEY_S] == 1)
		move_on_map(maps, 's', KEY_MOVE);
	if ((*maps)->key_pressed[KEY_D] == 1)
		move_on_map(maps, 'd', KEY_MOVE);
